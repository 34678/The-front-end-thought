        ...
        <section :class='$style.content'>
            <div :class='$style.item' v-for='item in items' :key='item.catory'>
                ...
                <section :class='$style.list'>
                    <div :class='$style.product' :key='product.img' v-for='product in item.list'>
                        <img :src='product.img' alt=''>
                        <p :class='$style.title'>{{ product.title }}</p>
                        <p :class='$style.price'><em>{{ product.price }}</em>起</p>
                        <article>
                            <!--为啥这里需要一个wrapper？因为需要根据他的长度占整体的比例?这样不就超过整个长度的比例了吗-->
...
<style lang="scss" module>
    ...
    .panel {
        ...
        .content {
            @include flex;
            .item:first-child .product:first-child label{
                display:block;
            }
            .item {
                > img {
